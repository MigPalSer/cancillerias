...
public class ServicioCombate {
	...
	public static void combateTerrestre1a1(Bandera banda_atacante, Bandera banda_defensora) {
		...
		//Extraemos los valores de cu�nta infanteria tiene cada bando, servir�n para las mejora
		int infanteria_atacante=banda_atacante.numeroInfanterias();
		int infanteria_defensora=banda_defensora.numeroInfanterias();		

		//Calculamos cu�ntos impactos hace cada bandera
	...
	//M�todo encargado de realizar combates terrestres entre dos grupos de banderas
	...
	public static void combateAereo(Bandera banda_atacante, Bandera banda_defensora) {
		...
		do {
			...
			//Aplicamos las bajas o las situamos a 0 si han sobrepasado el n�mero
	...
	public static void combateAereoMultiple(Set<Bandera> bandas_atacantes, Set<Bandera> bandas_defensoras) {
		...
		ServicioMensajes.println("Combate aereo m�ltiple iniciado");
		...
		do {
			...
			//Creamos una lista de strings para distribuir los da�os al azar y la barajamos
	...
	//Este m�todo se utiliza para que si una bandera se ha quedado sin infanterias pero la otra a�n tiene (tras el combate) logre capturar todas las piezas de equipo
	...
	public static int calcularImpactosTotales(Bandera b, boolean atacante) {
		...
		for (String s : b.getTropas().keySet()) {
			int numero=b.get(s);
			if(numero>0) {
				//Reduccion del n�mero de equipos que intervienen, maximo por infanterias
